...
class Functions
{
	...
	public static function get_results_vars_values() {
		...
		if ( isset( $wp_query->query_vars['in-taxonomy'] ) ) $in_taxonomy = wp_unslash( wp_strip_all_tags( urldecode( $wp_query->query_vars['in-taxonomy'] ) ) );
		elseif ( isset( $_REQUEST['in_taxonomy'] ) && trim( $_REQUEST['in_taxonomy'] ) != '' ) $in_taxonomy = wp_unslash( wp_strip_all_tags( $_REQUEST['in_taxonomy'] ) );
